...
class AccountJournal(models.Model):
    ...
    def _get_afip_ws(self):
        return [
            ("wsfe", self.env._("Domestic market -without detail- RG2485 (WSFEv1)")),
            ("wsfex", self.env._("Export -with detail- RG2758 (WSFEXv1)")),
            ("wsbfe", self.env._("Fiscal Bond -with detail- RG2557 (WSBFE)")),
        ]

    def _get_l10n_ar_afip_pos_types_selection(self):
        res = super()._get_l10n_ar_afip_pos_types_selection()
        res.insert(0, ("RAW_MAW", self.env._("Electronic Invoice - Web Service")))
        res.insert(3, ("BFEWS", self.env._("Electronic Fiscal Bond - Web Service")))
        res.insert(5, ("FEEWS", self.env._("Export Voucher - Web Service")))
        return res
