...
class AccountMove(models.Model):
    ...
    def _build_afip_wsfe_header(self):
        ...
        if invoice.get("fecha_serv_hasta"):
    ...
    def _build_afip_wsfe_other_taxes(self):
        ...
        for tax in not_vat_taxes:
            tribute_code = tax.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code
            base_lines = self.invoice_line_ids.filtered(
                lambda x, code=tribute_code: x.tax_ids.filtered(
                    lambda y: y.tax_group_id.l10n_ar_tribute_afip_code == code
                )
            )
            base_imp = sum(base_lines.mapped("price_subtotal"))
            other_taxes.append(
                {
                    "tributo_id": tribute_code,
                    "ds": tax.tax_line_id.tax_group_id.name,
                    "base_imp": f"{base_imp:.2f}",
                    "alic": 0.00,
                    "importe": f"{abs(tax.amount_currency):.2f}",
                }
            )
        return other_taxes
