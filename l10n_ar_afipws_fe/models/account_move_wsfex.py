...
class AccountMove(models.Model):
    ...
    def _build_afip_wsfex_header(self):
        ...
        if not country_id:
            raise UserError(
                self.env._(
                    'For WS "%(ws)s" country is required on partner',
                    ws=self.journal_id.afip_ws,
                )
            )
        elif not country_id.code:
            raise UserError(
                self.env._(
                    'For WS "%(ws)s" country code is mandatory. Country: "%(country)s"',
                    ws=self.journal_id.afip_ws,
            ...
            raise UserError(
                self.env._(
                    'For WS "%(ws)s" AFIP country code is mandatory. '
                    'Country: "%(country)s"',
                    ws=self.journal_id.afip_ws,
                    country=country_id.name,
                )
        ...
        if invoice["nro_doc"]:
            ...
            if not invoice["cuit_pais_cliente"]:
                raise UserError(
                    self.env._(
                        "No vat defined for the partner and also no CUIT set on country"
                    )
                )
