...
def moneyfmt(value, places=2, ndigits=15,
             curr='', sep='', dp='',
             pos='', neg='-', trailneg=''):
...
class FixedWidth(object):
    ...
    def __init__(self, config, **kwargs):
        ...
        for key, value in self.config.items():
            ...
            if value['end_pos'] < value['start_pos']:
                raise ValueError("%s end_pos must be *after* start_pos." %
                                 (key,))

            # make sure authorized type was provided
            if not value['type'] in (
                    'string', 'integer', 'decimal', 'numeric'):
        ...
        for start_pos, field_name in self.ordered_fields:
            ...
            if start_pos != current_pos:
                raise ValueError(
                    "Field %s starts at position %d; should be %d " +
                    "(or previous field definition is incorrect)."
        ...
        for field_name, parameters in self.config.items():
            ...
            if field_name in self.data:
                ...
                if not type_tests[parameters['type']](self.data[field_name]):
                    raise ValueError(
                        "%s is defined as a %s, but the value " +
                        "is not of that type."
                ...
                if parameters['type'] == 'string':
                    if len(self.data[field_name]) > parameters['length']:
                        self.data[field_name] = self.data[field_name][
                            :parameters['length']-1]
                ...
                if 'value' in parameters and \
                        parameters['value'] != self.data[field_name]:
    ...
    def _string_to_dict(self, fw_string):
        ...
        for start_pos, field_name in self.ordered_fields:
            ...
            self.data[field_name] = conversion[
                self.config[field_name]['type']](
                    fw_string[start_pos - 1:self.config[
                        field_name]['end_pos']])
