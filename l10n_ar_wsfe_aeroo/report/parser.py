...
#        super(Parser, self).set_context(objects, data, ids, report_type)
    ...
    def get_lines(self, inv):
        ...
        for line in lines:
            ...
            # Para PLOT
            ll.append([code, desc, q, round(pu,2), discount, discount2, subtotal, line.product_id.uom_id.name ])
            #ll.append([code, desc, q, pu, discount, discount2, subtotal])

        return ll

    def get_taxes(self, inv):
        taxes = []
        vat_disc = inv.denomination_id.vat_discriminated
        if not vat_disc:
            return taxes
        ...
        for tax in inv.tax_line:
            taxname = tax.name.split('-')[0].rstrip()
            t = [taxname, tax.base, tax.amount]
            taxes.append(t)
        return taxes
