    ...
    def _compute(self, invoice, base, amount):
        ...
        if invoice.type in ('out_invoice', 'in_invoice'):
            base_amount = currency.compute(
                base * tax.base_sign, company_currency, round=False)
            tax_amount = currency.compute(
                amount * tax.tax_sign, company_currency, round=False)
        else:  # invoice is out_refund
            base_amount = currency.compute(
                base * tax.ref_base_sign, company_currency, round=False)
            tax_amount = currency.compute(
                amount * tax.ref_tax_sign, company_currency, round=False)
...
class AccountInvoiceTax(models.Model):
    ...
    # Aca le agregamos las account_invoice_taxes
    # pertenecientes a las Percepciones
    # TODO
    # @api.v8
    ...
    #         tax_amount, base_amount = line._compute(
    #             invoice, val['base'], val['amount'])
